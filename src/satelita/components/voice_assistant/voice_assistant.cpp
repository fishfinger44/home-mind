...
void VoiceAssistant::start_playback_timeout_() {
  // 🔴 NASZA ZMIANA: 2000 -> 8000 ms (19.08.2026).
  // Zmierzone na potoku HA: satelita daje syntezie DOKŁADNIE 2 s na wydanie pierwszego
  // dźwięku, licząc od `tts_start_streaming`. Gdy model pisze dłużej, timeout uznaje
  // odpowiedź za zakończoną, otwiera kolejną turę — a ta UBIJA przebieg wciąż
  // generujący tekst. Objaw: odpowiedź urwana w pół zdania, żadnego `tts-start`,
  // do lektora nie idzie ani jedno żądanie, użytkownik nie słyszy nic.
  //   19:37:58.041  tts_start_streaming: True
  //   19:38:00.109  run-end            <- 2,068 s
  // Model skończył 2,36 s po zapowiedzi, czyli spóźnił się o trzy dziesiąte sekundy.
  // 8 s daje zapas na dłuższe odpowiedzi, a nadal chroni przed zawieszeniem, gdy
  // synteza naprawdę padnie.
  this->set_timeout("playing", 8000, [this]() {
    this->cancel_timeout("speaker-timeout");
    this->set_state_(State::RESPONSE_FINISHED, State::RESPONSE_FINISHED);
