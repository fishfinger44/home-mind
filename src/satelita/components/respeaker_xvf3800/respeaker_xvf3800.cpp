...
void RespeakerXVF3800::lock_beam() {
  ...
  this->beam_azimuth_captured_ = true;

  ESP_LOGI(TAG, "Beam locked at %.3f rad (%.1f deg)", radians, radians * 180.0f / (float)M_PI);
}

void RespeakerXVF3800::relock_beam() {
  if (this->beam_locked_)
    return;
  // Nothing captured yet: re-arming here would pin the beams at 0 rad.
  if (!this->beam_azimuth_captured_)
    return;

  // The azimuth written by lock_beam() is still in AEC_FIXEDBEAMS_AZIMUTH_CMD —
  // unlock_beam() only clears the on/off flag — so flipping it back on restores
  // the wake-word direction without reading the current (possibly noise) one.
  uint8_t on[4] = {0x01, 0x00, 0x00, 0x00};
  this->xmos_write_bytes(AEC_SERVICER_RESID, AEC_FIXEDBEAMS_ONOFF_CMD, on, sizeof(on));

  this->beam_locked_ = true;

  ESP_LOGI(TAG, "Beam re-locked at the captured wake-word azimuth");
}
