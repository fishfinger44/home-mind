...
**HA tool definitions** are provider-neutral `ToolDefinition[]` in `llm/tool-definitions.ts`, converted to provider format via `toAnthropicTools()` / `toOpenAITools()` / `toGeminiTools()`. Nine tools: `get_state`, `get_entities`, `search_entities`, `call_service`, `get_history`, `web_search`, `zaplanuj_trase`, `odjazdy`, `sprawdz_pamiec`. Shared execution logic in `llm/tool-handler.ts`.

**Route planning** (`zaplanuj_trase`, `src/trasy/`) answers "how do I get to X by public transport" against Google Routes API, called directly rather than through Home Assistant. HA's own `google_travel_time.get_transit_times` cannot answer it: its field mask is `routes.duration,routes.distanceMeters,routes.localized_values`, so it never asks Google for the legs of the journey and returns duration and distance only. The failure that motivated this was the assistant naming line 111 *in the wrong direction* — direction cannot be inferred from where the house is, it needs a second point. `routes.legs.steps.transitDetails` carries `transitLine.nameShort` (line number), **`headsign` (the direction, spoken aloud instead of guessed)**, the boarding stop and the times. That field also sets the price tier — it is the whole reason for the call.

**The departure board is a separate tool** (`odjazdy`, `src/trasy/odjazdy.ts`), because these are two different questions and one tool answering both is what broke: Routes API insists on a *destination*, so asked "when does the next 111 leave" the model put the line number in `dokad`, Google geocoded the nonsense, and a false "no connection found" came back looking like a broken timetable. `odjazdy` reads wrocław.pl instead — the city renders the timetable server-side as plain text ("Odjazd 29 minut po godzinie 16") in three day variants, and the line's own page lists every stop as a slug carrying stop, direction and post number, so the direction is matched rather than guessed. Verified against Google to the minute. Web search is not a substitute, though the reason is narrower than it first looked: a grounded Gemini micro-call *does* return the timetable and matched the page exactly for 15 of 20 hours — but it collapsed the 9–13 block into prose and silently dropped the 22:13 service, so asking at 22:11 yields 22:24 with nothing to signal the loss. (Tavily, the fallback once the grounded key is out of the chain, returns links and no times at all. Note the search key lives only in `/data/llm-override.json`; calling `runWebSearch` without passing it measures a different backend than conversations use.) No key, no quota, pages cached `ODJAZDY_CACHE_H` hours (default 6). Picking the wrong day section would state the time of a service that does not run today, so the section is selected by the *home* timezone's weekday, not the server's.

This is not a departure board: Routes API answers A→B at a given time, not "the next three 111s". Nor does it know live delays — Wrocław publishes raw GPS positions but no GTFS-RT, so Google has nothing to feed them from. Planned changes (roadworks, detours, moved stops) *are* in the city's GTFS, which Google refreshes every 24 h, so those it does see. Every result carries a `zrodlo` note saying so, because the assistant otherwise oversells the freshness.

Two brakes, because the caller is an LLM and a loop can fire hundreds of requests: a quota cap set in the Google console (which actually refuses requests — a budget alert only notifies after the fact), and `TRASY_LIMIT_DZIENNY` here, counted in `/data/trasy-uzycie.json` **before** each request so a run of failing calls is stopped too. `TRASY_MIEJSCA` maps spoken shorthands (`dom`, `praca`, `centrum`) onto addresses; `dom` is the default origin, and an unknown name is passed to Google as an address rather than rejected.

Everything configurable is editable from the **`/trasy` panel** (sidebar entry "Trasy" in HA), which writes `/data/trasy-config.json`. That file overrides `.env` per field and survives a restart — same arrangement as `/data/llm-override.json`. Places are overridden **wholesale** rather than merged: merging would make deleting an entry impossible, since the `.env` copy would silently come back. The key can be set from the panel but never read back out of it.
...
Route planning (optional): `GOOGLE_ROUTES_API_KEY` (Google Maps Platform key with **Routes API enabled** — a different product from the free Gemini/AI Studio key, and it needs a billing account), `TRASY_MIEJSCA` (JSON of shorthand → address; the `dom` key is the default origin), `TRASY_LIMIT_DZIENNY` (default 100, `0` disables the tool), `TRASY_ILE_WARIANTOW` (default 3), `TRASY_TIMEOUT_MS`, `TRASY_JEZYK`, `TRASY_REGION`, `TRASY_UZYCIE_PATH`. The departure board needs no key: `TRASY_PRZYSTANEK_DOMYSLNY` (the stop by the house, default `Waniliowa`), `ODJAZDY_CACHE_H` (default 6), `ODJAZDY_TIMEOUT_MS`. Without the key the tool stays listed but returns an error telling the model to say it cannot check and **not** to invent line numbers.
...
- `GET|PUT /api/trasy` — Read/save route-planning settings (places, daily limit, alternatives, API key). GET **never returns the key** — only `maKlucz` and `zrodloKlucza` (`panel` | `env` | `brak`), because the panel sits on an open LAN. PUT with `klucz: ""` drops the stored key and falls back to `.env`.
- `POST /api/trasy/sprawdz` — One test route from the panel. Costs exactly what a spoken question costs: one Routes API call and one point of the daily limit.
- `GET /trasy` — The panel page itself (mounted outside `/api`, like `/rules` and `/pamiec`, so the HA sidebar can point an iframe at it).
