...
**HA tool definitions** are provider-neutral `ToolDefinition[]` in `llm/tool-definitions.ts`, converted to provider format via `toAnthropicTools()` / `toOpenAITools()` / `toGeminiTools()`. Seven tools: `get_state`, `get_entities`, `search_entities`, `call_service`, `get_history`, `web_search`, `zaplanuj_trase`. Shared execution logic in `llm/tool-handler.ts`.
