...
- **`MAX_TUR_BEZ_DZIALANIA = 2`** — po dwóch turach z rzędu bez ani jednego wywołania usługi nasłuch się zamyka. Jedyne zabezpieczenie tutaj, które nie zależy od języka, treści odpowiedzi ani od tego, czy model posłuchał reguł; zamyka też pętlę, której nie przewidziano. Wymagało, by `_call_api()` zwracało `toolsUsed`, dotąd odrzucane. Akcja zeruje licznik — łańcuch poleceń może trwać dowolnie długo, w tym przez agenta lokalnego.
